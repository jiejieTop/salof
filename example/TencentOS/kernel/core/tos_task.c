...
__API__ k_err_t tos_task_destroy(k_task_t *task)
{
    ...
    if (task == k_curr_task && knl_is_sched_locked()) {
...
__API__ k_err_t tos_task_suspend(k_task_t *task)
{
    ...
    if (unlikely(task == k_curr_task)) {
        if (knl_is_sched_locked()) { // if not you, who?
            TOS_CPU_INT_ENABLE();
            return K_ERR_SCHED_LOCKED;
        }
    }
...
__API__ k_err_t tos_task_delay_abort(k_task_t *task)
{
    ...
    if (task == k_curr_task || !task_state_is_sleeping(task)) {
        TOS_CPU_INT_ENABLE();
